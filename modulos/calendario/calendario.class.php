			...
			for($i=0;$i<$num;$i++){
				...
				?>
			  	<tr>
			     	...
			     	<td class=""><?php echo $this->fmt->usuario->nombre_grupo_usuario($fila["cal_grupo_usuario"]); ?></td>
			      <td class="td-user col-xl-offset-2 acciones">
							<a  id="btn-restaurar-modulo" class="btn btn-accion btn-restaurar" href="<? echo $url; ?>" title="Editar <? echo $fila["cal_id"]; ?>" ><i class="icn-sync"></i></a>
 						<a  title="Eliminar <? echo $fila["cal_nombre"]; ?>" type="button" idEliminar="<? echo $fila["cal_id"]; ?>" nombreEliminar="<? echo $fila["cal_nombre"]; ?>" tarea="vaciar_id" class="btn btn-eliminar btn-accion"><i class="icn-trash"></i></a>
			      </td>
			  	</tr>
			  	<?php
			  		}
