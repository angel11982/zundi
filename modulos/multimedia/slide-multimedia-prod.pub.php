...
$sql="SELECT DISTINCT mod_pro_mul_ruta, mod_pro_mul_dominio FROM mod_productos_mul WHERE mod_pro_mul_id_prod=".$fila['mod_prod_id'];
					...
					if ($num>0){
						...
						for($i=0;$i<$num;$i++){
							list($fila_archivo, $fila_dominio)=$this->fmt->query->obt_fila($rs);

							if (empty($fila_dominio)){ $aux=_RUTA_WEB; } else { $aux = $this->fmt->categoria->traer_dominio_cat_id($fila_dominio); }
							$ext=$this->fmt->archivos->saber_extension_archivo($fila_archivo);
							$archivo = $aux . $fila_archivo;
							switch ($ext) {
							    case "mp4":
							       	$nombre_archivo=$this->fmt->archivos->saber_nombre_archivo($fila_archivo);
									$magenfile = $aux ."sitios/mainter/archivos/productos/". $nombre_archivo . ".jpg";
									$imagenFile= $this->fmt->archivos->convertir_nombre_thumb($magenfile);
									$type="video";
							        break;
							    case "mp3":

							        $imagenFile = $aux . "sitios/mainter/archivos/productos/audio.png";
									$type="music";
							        break;
							    default:
